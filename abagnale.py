...
def unzip_transactions_file(file_name):
...
def load_image1(png_file):
    data  = Image.open(png_file)
    return data

@st.cache(allow_output_mutation=True)
def load_image2(png_file):
    data  = Image.open(png_file)
    return data


@st.cache(allow_output_mutation=True)
def get_transactions(file_prefix):
    # assigning the filename to a variable
    zipped_file_name = "".join([file_prefix, ".", "zip"])
    unzipped_file_name = "".join([file_prefix, ".", "zip"])
    print(f"zipped_file = {zipped_file_name}")
    print(f"unzipped_file = {unzipped_file_name}")


    if not os.path.exists(unzipped_file_name):
        unzip_transactions_file(zipped_file_name)
    ...
    # parse the in line-delimited JSON file like a CSV using JSON parser to parse each line 
    with open('transactions_shrunk.txt') as csv_file:
...
def load_rf_grid_model():
    ...
    with zipfile.ZipFile('rf_grid.sav.zip', 'r') as file:
        file.extractall()
    
    # load trained rf model
    rf_grid_model = pickle.load(open('rf_grid.sav', 'rb'))
    return  rf_grid_model


@st.cache(allow_output_mutation=True)
def load_logistic_grid_model():
    # load trained svm model 
    logistic_grid_model = pickle.load(open('logistic_grid.sav', 'rb'))
    return  logistic_grid_model
...
def main():
    ...
    # Read the data
    df = get_transactions("transactions_shrunk")
    ...
    model_type = st.sidebar.selectbox("Recommendation Model", ["", "Logistic Regression", "Random Forest"])
    if model_type == "Logistic Regression":
        st.subheader("Accuracy")
        logistic_grid_model = load_logistic_grid_model()
        result = logistic_grid_model.score(X_test_std, y_test)
        st.write(result)
        ...
        st.image(load_image1('cost_savings_2.png'))
